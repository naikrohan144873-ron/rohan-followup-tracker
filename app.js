/* app.js - main logic */
(() => {
  const LS_KEY = 'rohan_tasks_v2';
  const LS_SETTINGS = 'rohan_settings_v2';
  ...
  function closeModal(){ const root=document.getElementById('modalRoot'); if(root){ root.innerHTML=''; root.style.display='none'; } }
  ...
  function populateSelects(){
    // index filters
    const compSel = qs('#filterCompany') || qs('#company');
    if(compSel){
      // avoid duplicates
      const existing = Array.from(compSel.options).map(o=>o.value);
      companies.forEach(c=>{ if(!existing.includes(c)){ const o=document.createElement('option'); o.value=c; o.textContent=c; compSel.appendChild(o);} });
    }
    const deptSel = qs('#filterDept') || qs('#department');
    if(deptSel){
      const existing = Array.from(deptSel.options).map(o=>o.value);
      departments.forEach(d=>{ if(!existing.includes(d)){ const o=document.createElement('option'); o.value=d; o.textContent=d; deptSel.appendChild(o);} });
    }

    // Also populate company/department selects from localStorage data (if user added new)
    const tasks = getTasks();
    tasks.forEach(t=>{
      if(t.company && !companies.includes(t.company)) companies.push(t.company);
      if(t.department && !departments.includes(t.department)) departments.push(t.department);
    });

    // re-render selects fully (simple approach)
    ['#filterCompany','#company'].forEach(sel=>{
      const el = qs(sel);
      if(!el) return;
      // keep first option text
      const first = el.querySelector('option') ? el.querySelector('option').textContent : '';
      el.innerHTML = first ? `<option value="">${first}</option>` : '<option value="">All</option>';
      companies.forEach(c => { const o = document.createElement('option'); o.value=c; o.textContent=c; el.appendChild(o); });
    });
    ['#filterDept','#department'].forEach(sel=>{
      const el = qs(sel);
      if(!el) return;
      const first = el.querySelector('option') ? el.querySelector('option').textContent : '';
      el.innerHTML = first ? `<option value="">${first}</option>` : '<option value="">All</option>';
      departments.forEach(d => { const o = document.createElement('option'); o.value=d; o.textContent=d; el.appendChild(o); });
    });
  }
  ...
  function renderDashboard(){
    ...
    let list = tasks.slice().sort((a,b) => {
      const da = dateTimeToDateObj(a.followDate,a.followTime) || new Date(a.createdAt || a.id.replace('t_',''));
      const db = dateTimeToDateObj(b.followDate,b.followTime) || new Date(b.createdAt || b.id.replace('t_',''));
      return da - db;
    });
    ...
    list.forEach(t=>{
      ...
      div.innerHTML = `
        <div class="task-top">
          <div>
            <div class="task-title">${escapeHtml(t.personName||'(no name)')}</div>
  ...
  function wireAdd(){
    ...
    if(form){
      form.addEventListener('submit', (ev)=>{
        ev.preventDefault();
        const t = addTaskFromForm(form);
        // Keep newly added project/department available
        if(t.company && !companies.includes(t.company)) companies.push(t.company);
        if(t.department && !departments.includes(t.department)) departments.push(t.department);
    ...
    if(newProjectBtn) newProjectBtn.addEventListener('click', ()=>{
      ...
      if(name){ companies.push(name); saveSettings(getSettings()); populateSelects(); }
    });
    const newDeptBtn = qs('#newDeptBtn');
    if(newDeptBtn) newDeptBtn.addEventListener('click', ()=>{
      const name = prompt('Enter new department name:');
      if(name){ departments.push(name); saveSettings(getSettings()); populateSelects(); }
    });
  }
  ...
  function wireList(){
    ...
    const navHome2 = qs('#navHome'); if(navHome2) navHome2.addEventListener('click', ()=> location.href='/index.html');
  ...
  function scheduleNotifications(){
    ...
    setInterval(async ()=>{
      ...
      if(hhmm === reminderTime && lastFired !== hhmm){
        ...
        const body = tasks.slice(0,5).map(t=>`${t.personName || '(no name)'} @ ${t.company||''} ${t.followTime||''}`).join('\n') || 'No tasks';
      ...
      } else if(hhmm !== reminderTime && settings._lastFired){
